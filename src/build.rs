...
pub async fn execute(build_id: Uuid, params: NewBuild, db: Db, webhook_url: Option<String>) {
    ...
    let payload = match &result {
        ...
        Err(e) => {
            if let Err(err) = db.mark_build_failed(build_id, &e.to_string()).await {
    ...
    if let Err(e) = &result {
        ...
        if let Ok(pid) = Pubkey::from_str(&program_id) {
            if crate::onchain::is_program_buffer_missing(&pid).await {
                if let Err(err) = db.mark_closed(&program_id).await {
